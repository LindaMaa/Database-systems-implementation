...
// WanderJoin version with smoothing
...
private:
    ...
    // sample next tuple, use Lidstone smoothing
    ...
    {
        int totalCount;
        const double cc = 0.000001; // very small count
        ...
        if (totalCount == 0)
        {
            multi = cc / (cc * (p.loaded_relations_totals[right_table_index] + 1));
            prob = 1.0;
            return multi; // no match found but return non-zero count
        }
        ...
        long double smoothed_multi = multi + cc;
        long double smoothed_total_count = totalCount + cc * (p.loaded_relations_totals[right_table_index] + 1);

        prob = smoothed_multi / smoothed_total_count;
...
public:
    const long double computeEstimate(int num_samples, Constraints &constr, vector<unordered_map<tuple<int, int>, int, hash_tuple>> &hashMaps, Parser &p)
    {
        ...
        if (size == 0)
        {
            ...
            std::cout << "WJ SMOOTH -> Estimated number of answers: 0" << endl;
            cout << "WJ SMOOTH -> Estimation time: " << time_taken << " ms" << endl;
            return 0;
        }
        ...
        std::cout << "WJ with smoothing -> Estimated number of answers: " << averageEstimate << endl;
        cout << "WJ with smoothing  -> Estimation time: " << time_taken << " ms" << endl;
        std::cout << "WJ with smoothing -> Number of samples taken: " << num_samples << endl;
